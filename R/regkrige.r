...
regkrige<-function(x=NULL, y=NULL, z=NULL, field = NULL, edge = 0, filter = 1, resolution = NULL,
  ...
  if(cross.validate){
    for (i in 1:nrow(z)){
      gsmod<-gstat(formula=regres~1, locations=~x+y, data=z.df[-i,], model=vgmod)
      z[i,'regkrig_cv']<-predict(gsmod, z.df[i,],debug.level=0)$var1.pred
    }
    z$regkrig_cv<-z$regpred-z$reskrig_cv
  }

  #residual kriging to raster
  crs(x)<-crs(z) #to fix a bug
  gsmod<-gstat(formula=regres~1, locations=~x+y, data=z.df, model=vgmod)
  reskrigpred<-interpolate(x, gsmod,  fun=predict, debug.level=0)$var1.pred
  regpred<-predict(x, reg)
  regkrig<-regpred-reskrigpred
  regkrig<-mask(regkrig, x)
